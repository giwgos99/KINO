...
class KinoGUI:
    ...
    def lottery_animation(self):
        for number in self.win_list:
            self.buttons[number].configure(bg="green",fg="white")

    def reset_button(self):
        for button in self.buttons:
            self.buttons[button].configure(bg="#004488", fg="white")

    def lottery_draw(self):
        self.win_list = random.sample(range(1, 81), 20)
        self.lottery_animation()
        #print(str(self.win_list))
    #-----------------------------------------------
